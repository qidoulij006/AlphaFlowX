	...
	if err == nil && data != nil {
		...
		if err := json.Unmarshal(data, &orders); err == nil {
			...
			for _, order := range orders {
				...
				if instErr == nil && inst != nil && inst.CtVal > 0 {
					quantity *= inst.CtVal
				}
...
func (t *OKXTrader) PlaceLimitOrder(req *types.LimitOrderRequest) (*types.LimitOrderResult, error) {
	...
	// OKX dual-side mode requires the caller's intended position side to be
	// preserved. Deriving posSide from BUY/SELL would turn a close-long sell
	// into an open-short order.
	side := "buy"
