...
func (t *FuturesTrader) PlaceLimitOrder(req *types.LimitOrderRequest) (*types.LimitOrderResult, error) {
	...
	// Binance futures hedge mode rejects the reduceOnly parameter on regular limit
	// orders. Grid exits are still expressed correctly by side + positionSide + qty.
	// Sending ReduceOnly(false) also serializes the field and triggers -1106.
