...
var (
	...
	binanceSyncControl      = make(map[string]*syncControlState) // exchangeID -> sync control state
	binanceSyncControlMutex sync.Mutex
)

const binanceSyncMinGap = 2 * time.Minute

type syncControlState struct {
	inFlight    bool
	lastAttempt time.Time
	banUntil    time.Time
}

func acquireBinanceSyncSlot(exchangeID string) bool {
	binanceSyncControlMutex.Lock()
	defer binanceSyncControlMutex.Unlock()

	state, exists := binanceSyncControl[exchangeID]
	if !exists {
		state = &syncControlState{}
		binanceSyncControl[exchangeID] = state
	}

	now := time.Now().UTC()
	if state.inFlight {
		logger.Infof("⏭️ Binance order sync skipped for %s: another sync is already in progress", exchangeID)
		return false
	}
	if !state.banUntil.IsZero() && now.Before(state.banUntil) {
		logger.Infof("⏭️ Binance order sync skipped for %s: upstream banned until %s",
			exchangeID, state.banUntil.Format(time.RFC3339))
		return false
	}
	if !state.lastAttempt.IsZero() && now.Sub(state.lastAttempt) < binanceSyncMinGap {
		logger.Infof("⏭️ Binance order sync skipped for %s: last attempt was %.0fs ago",
			exchangeID, now.Sub(state.lastAttempt).Seconds())
		return false
	}
...
func releaseBinanceSyncSlot(exchangeID string) {
	binanceSyncControlMutex.Lock()
	defer binanceSyncControlMutex.Unlock()

	if state, exists := binanceSyncControl[exchangeID]; exists {
		state.inFlight = false
	}
}

func updateBinanceSyncBan(exchangeID string, err error) {
	banUntil := parseBinanceBanUntil(err)
	if banUntil.IsZero() {
		return
	}
	...
	if banUntil.After(state.banUntil) {
...
func (t *FuturesTrader) SyncOrdersFromBinance(traderID string, exchangeID string, exchangeType string, st *store.Store) error {
	...
	if !acquireBinanceSyncSlot(exchangeID) {
		return nil
	}
	defer releaseBinanceSyncSlot(exchangeID)
	...
	if err != nil {
		updateBinanceSyncBan(exchangeID, err)
	...
	for _, symbol := range changedSymbols {
		...
		if queryErr != nil {
			updateBinanceSyncBan(exchangeID, queryErr)
...
func (t *FuturesTrader) StartOrderSync(traderID string, exchangeID string, exchangeType string, st *store.Store, interval time.Duration) {
	...
	if cooldownUntil := t.GetCooldownUntil(); !cooldownUntil.IsZero() {
		if remaining := time.Until(cooldownUntil.Add(15 * time.Second)); remaining > initialDelay {
			initialDelay = remaining
		}
	}

	// Delay the first sync so startup does not immediately re-trigger an upstream ban.
	go func() {
		if initialDelay > 0 {
			logger.Infof("⏳ Delaying initial Binance order sync by %v", initialDelay)
