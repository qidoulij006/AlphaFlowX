...
func (t *FuturesTrader) GetBalance() (map[string]interface{}, error) {
	if snapshot, ok := t.getUserStreamBalanceSnapshot(); ok {
		logger.Debugf("Using Binance user stream balance snapshot")
		return snapshot, nil
	}

	if err := t.checkCooldown(); err != nil {
		t.balanceCacheMutex.RLock()
		hasCachedBalance := t.cachedBalance != nil
		cachedBalance := t.cachedBalance
		cacheAge := time.Since(t.balanceCacheTime)
		t.balanceCacheMutex.RUnlock()

		if hasCachedBalance {
			logger.Infof("⚠️ Binance cooldown active, returning cached balance (cache age: %.1f seconds)", cacheAge.Seconds())
	...
	if err != nil {
		...
		if hasCachedBalance && isBinanceRateLimitError(err) {
			logger.Infof("⚠️ Binance account API rate-limited, returning stale cached balance (cache age: %.1f seconds): %v",
