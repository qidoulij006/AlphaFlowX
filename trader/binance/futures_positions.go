...
func (t *FuturesTrader) GetPositions() ([]map[string]interface{}, error) {
	if snapshot, ok := t.getUserStreamPositionSnapshot(); ok {
		logger.Debugf("Using Binance user stream position snapshot")
		return snapshot, nil
	}

	if err := t.checkCooldown(); err != nil {
		t.positionsCacheMutex.RLock()
		hasCachedPositions := t.cachedPositions != nil
		cachedPositions := t.cachedPositions
		cacheAge := time.Since(t.positionsCacheTime)
		t.positionsCacheMutex.RUnlock()

		if hasCachedPositions {
			logger.Infof("⚠️ Binance cooldown active, returning cached positions (cache age: %.1f seconds)", cacheAge.Seconds())
	...
	if err != nil {
		...
		if hasCachedPositions && isBinanceRateLimitError(err) {
			logger.Infof("⚠️ Binance positions API rate-limited, returning stale cached positions (cache age: %.1f seconds): %v",
				cacheAge.Seconds(), err)
			return cachedPositions, nil
		}
	...
	t.syncUserStreamPositionMetadata(result)
