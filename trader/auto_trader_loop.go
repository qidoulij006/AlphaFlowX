...
const maxLoggedAIDebugText = 2048

func summarizeLoggedText(label, value string) string {
	if value == "" {
		return fmt.Sprintf("%s: <empty>", label)
	}
	if len(value) <= maxLoggedAIDebugText {
		return fmt.Sprintf("%s (%d bytes): %s", label, len(value), value)
	}
	return fmt.Sprintf("%s (%d bytes, showing first %d): %s", label, len(value), maxLoggedAIDebugText, value[:maxLoggedAIDebugText])
}
...
func (at *AutoTrader) runCycle() error {
	...
	if err != nil {
		...
		if aiDecision != nil {
			logger.Info("\n" + strings.Repeat("=", 70) + "\n")
			logger.Infof("📋 AI debug output suppressed to bounded preview (error case)")
			logger.Info(strings.Repeat("=", 70))
			logger.Info(summarizeLoggedText("System prompt", aiDecision.SystemPrompt))
			logger.Info(strings.Repeat("=", 70))

			if aiDecision.CoTTrace != "" {
				logger.Info("\n" + strings.Repeat("-", 70) + "\n")
				logger.Info("💭 AI reasoning preview (error case):")
				logger.Info(strings.Repeat("-", 70))
				logger.Info(summarizeLoggedText("CoT trace", aiDecision.CoTTrace))
				logger.Info(strings.Repeat("-", 70))
			}
		}
...
func (at *AutoTrader) buildTradingContext() (*kernel.Context, error) {
	...
	if err != nil {
		if cached, ok := at.getCachedAccountInfoOnRateLimit(err, "buildTradingContext balance"); ok {
			balance = map[string]interface{}{
				"totalWalletBalance":    cached["wallet_balance"],
				"totalUnrealizedProfit": cached["unrealized_profit"],
				"availableBalance":      cached["available_balance"],
				"totalEquity":           cached["total_equity"],
			}
		} else {
