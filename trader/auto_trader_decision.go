...
const accountInfoCacheTTL = 30 * time.Second

func cloneAccountInfo(src map[string]interface{}) map[string]interface{} {
	if src == nil {
		return nil
	}
...
func (at *AutoTrader) getCachedAccountInfoSnapshot() (map[string]interface{}, time.Time) {
	at.accountInfoCacheMutex.RLock()
	defer at.accountInfoCacheMutex.RUnlock()
	return cloneAccountInfo(at.accountInfoCache), at.accountInfoCacheTime
}

func (at *AutoTrader) getCachedAccountInfoOnRateLimit(err error, operation string) (map[string]interface{}, bool) {
	if !isUpstreamRateLimitError(err) {
		return nil, false
	}

	cached, cachedAt := at.getCachedAccountInfoSnapshot()
	if cached == nil {
		return nil, false
	}

	cacheAge := time.Since(cachedAt).Seconds()
	logger.Infof("⚠️ [%s] %s rate-limited, using cached account info snapshot (cache age: %.1f seconds): %v",
		at.name, operation, cacheAge, err)
	return cached, true
}
...
func (at *AutoTrader) GetAccountInfo() (map[string]interface{}, error) {
	at.accountInfoCacheMutex.RLock()
	if at.accountInfoCache != nil && time.Since(at.accountInfoCacheTime) < accountInfoCacheTTL {
		cached := cloneAccountInfo(at.accountInfoCache)
		at.accountInfoCacheMutex.RUnlock()
		logger.Infof("✓ [%s] Using cached account info (cache age: %.1f seconds)", at.name, time.Since(at.accountInfoCacheTime).Seconds())
		return cached, nil
	}
	at.accountInfoCacheMutex.RUnlock()
	...
	if err != nil {
		if cached, ok := at.getCachedAccountInfoOnRateLimit(err, "GetAccountInfo balance"); ok {
			return cached, nil
		}
	...
	at.accountInfoCacheMutex.Lock()
	at.accountInfoCache = cloneAccountInfo(result)
	at.accountInfoCacheTime = time.Now()
	at.accountInfoCacheMutex.Unlock()
...
func (at *AutoTrader) recordPositionChange(orderID, symbol, side, action string, quantity, price float64, leverage int, entryPrice float64, fee float64) {
	...
	case "open_long", "open_short":
		...
		if err := at.store.Position().Create(pos); err != nil {
			...
			sendTradeEventEmailAsync(at.store, at.userID, at.id, at.name, action, symbol, side, quantity, price, leverage, fee, orderID)
		}
