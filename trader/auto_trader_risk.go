...
func (at *AutoTrader) checkPositionDrawdown() {
	...
	gridSymbol := ""
	if at.config.StrategyConfig != nil && at.config.StrategyConfig.StrategyType == "grid_trading" && at.config.StrategyConfig.GridConfig != nil {
		gridSymbol = strings.ToUpper(strings.TrimSpace(at.config.StrategyConfig.GridConfig.Symbol))
	}

	for _, pos := range positions {
		symbol := pos["symbol"].(string)
		if gridSymbol != "" && !strings.EqualFold(symbol, gridSymbol) {
			continue
		}
		...
		if currentPnLPct > 5.0 && drawdownPct >= 40.0 {
			...
			if at.config.StrategyConfig != nil && at.config.StrategyConfig.StrategyType == "grid_trading" &&
				at.config.StrategyConfig.GridConfig != nil && strings.EqualFold(symbol, at.config.StrategyConfig.GridConfig.Symbol) {
				gridSide := "LONG"
				if strings.EqualFold(side, "short") {
					gridSide = "SHORT"
				}
				if err := at.activateAggregatedRiskExit(gridSide, fmt.Sprintf("drawdown_monitor %.2f%%", drawdownPct)); err != nil {
					logger.Infof("❌ Drawdown aggregated risk exit failed (%s %s): %v", symbol, side, err)
				} else {
					logger.Infof("✅ Drawdown aggregated risk exit armed: %s %s", symbol, side)
					at.ClearPeakPnLCache(symbol, side)
				}
			} else if err := at.emergencyClosePosition(symbol, side); err != nil {
