#!/bin/bash
#
# 第一原则：严禁使用官方 GHCR 预构建镜像。
# 当前 AlphaFlowX 部署必须始终基于本地代码构建，与官方版本完全隔离。
#
# AlphaFlowX One-Click Installation Script
# Code repository: https://github.com/NoFxAiOS/nofx
#
# Usage:
#   curl -fsSL https://alphaflowx.com/install.sh | bash
#
# Or with custom directory:
#   curl -fsSL https://alphaflowx.com/install.sh | bash -s -- /opt/alphaflowx
#
...
generate_env() {
    ...
    # Create .env file
    cat > .env << EOF
# AlphaFlowX Configuration (Auto-generated)
...
# Prompt optimization email summary
PROMPT_OPT_DISPLAY_TIMEZONE=Asia/Shanghai
PROMPT_OPT_EMAIL_ENABLED=false
PROMPT_OPT_SMTP_HOST=smtp.163.com
PROMPT_OPT_SMTP_PORT=587
PROMPT_OPT_SMTP_USERNAME=
PROMPT_OPT_SMTP_PASSWORD=
PROMPT_OPT_EMAIL_FROM=
PROMPT_OPT_EMAIL_TO=
EOF
...
# Start services
start_services() {
    echo -e "${YELLOW}Starting AlphaFlowX services...${NC}"
    $COMPOSE_CMD up -d
...
print_success() {
    ...
    echo -e "╚════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "  ${GREEN}curl -fsSL https://alphaflowx.com/install.sh | bash${NC}"
    echo ""
