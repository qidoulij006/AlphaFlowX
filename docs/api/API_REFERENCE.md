...
### 基础信息
- Historical reference: this document describes a legacy external data API.
- Current AlphaFlowX deployment does not enable this external service.
- Legacy base URL: `https://nofxos.ai`
- **响应格式**: JSON
- **缓存时间**: 15秒（所有数据接口）
- **限流**: 每个IP每秒最多30次请求
