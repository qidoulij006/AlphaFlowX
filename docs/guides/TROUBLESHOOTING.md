...
#### ❌ OKX Error: `You can't complete this request under your current account mode`

**Symptoms:**
- Account balance is available, but every grid seed order or opening order fails
- Logs repeatedly show `OKX order failed: You can't complete this request under your current account mode`
- You may also see `Auto-seed completed with no new entries` or a high `placementFailures` count

**Root Cause:**
- NOFX sends OKX perpetual orders with `tdMode=cross` and `posSide=long/short`
- This requires an OKX account mode that supports hedge-style swap trading
- If the account is in `Portfolio Margin`, `net mode`, or another incompatible mode, OKX rejects every order

**Recommended checks:**
1. **Position Mode**
   - Open OKX futures settings
   - Confirm `Position Mode` is **Hedge Mode**
2. **Account Mode**
   - Do not use **Portfolio Margin** if it only supports `net mode`
   - Prefer **Futures mode** for perpetual trading
3. **Margin Mode**
   - Confirm the swap uses **Cross**
4. **Log pattern**
   - Look for this sequence:
   ```text
   OKX balance: Total equity=...
   PlaceLimitOrder: ... DOGE-USDT-SWAP ...
   OKX order failed: You can't complete this request under your current account mode
   Auto-seed completed with no new entries ... placementFailures=20
   ```

**Diagnosis:**
- If balance is available, orders are being attempted, and every order is rejected with this message, the problem is usually not insufficient funds and not missing AI decisions.
- The real issue is an **OKX account mode mismatch**.

**Fix:**
1. Set `Account Mode` to **Futures mode**
2. Set `Position Mode` to **Hedge Mode**
3. Use **Cross** margin for the swap
4. Restart the trader and verify the first order can be placed successfully

**Why NOFX needs this:**
- The OKX trader tries to switch to `long_short_mode`
- Leverage setup uses `mgnMode=cross`
- Grid orders are submitted with `posSide=long/short`
- The exchange account must support that combination
