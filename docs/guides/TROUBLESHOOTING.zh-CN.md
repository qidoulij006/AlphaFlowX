...
#### ❌ OKX 报错: `You can't complete this request under your current account mode`

**症状:**
- 账户余额正常，但网格补单或开仓连续失败
- 日志里反复出现 `OKX order failed: You can't complete this request under your current account mode`
- 最终可能看到 `Auto-seed completed with no new entries` 或大量 `placementFailures`

**根本原因:**
- NOFX 在 OKX 永续下单时固定使用 `tdMode=cross`，并带 `posSide=long/short`
- 这要求 OKX 账户支持双向持仓和对应的合约模式
- 如果账户处于 `Portfolio Margin`、`net mode` 或其他不兼容模式，交易所会直接拒单

**建议检查顺序:**
1. **持仓模式**
   - 打开 OKX 合约交易设置
   - 确认 `Position Mode` 为 **Hedge Mode / 双向持仓**
2. **账户模式**
   - 不要使用仅支持 `net mode` 的 **Portfolio Margin**
   - 优先使用可正常交易永续合约的 **Futures mode**
3. **保证金模式**
   - 确认合约使用 **Cross**
4. **日志确认**
   - 重点看是否同时出现以下组合：
...
**结论判断:**
- 如果余额正常、策略已开始挂单、但每一笔都被上述错误拒绝，问题通常不是资金不足，也不是 AI 未出信号，而是 **OKX 账户模式与系统下单模式不兼容**

**修复建议:**
1. 将 `Account Mode` 调整为 **Futures mode**
2. 将 `Position Mode` 调整为 **Hedge Mode**
3. 将合约保证金模式设置为 **Cross**
4. 重新启动交易员并观察是否已能成功下第一笔单

**代码侧行为说明:**
- OKX 交易器会尝试切到 `long_short_mode`
- 杠杆设置使用 `mgnMode=cross`
- 网格下单固定传 `posSide=long/short`
- 所以账户端必须与这一组假设一致
