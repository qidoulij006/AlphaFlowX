...
var (
	...
	reReasonKey    = regexp.MustCompile(`"reason"\s*:`)
)
...
func GetFullDecisionWithStrategy(ctx *Context, mcpClient mcp.AIClient, engine *StrategyEngine, variant string) (*FullDecision, error) {
	...
	if ctx.OITopDataMap == nil {
		...
		oiPositions, err := engine.externalDataClient.GetOITopPositions()
		if err == nil {
...
func extractDecisions(response string) ([]Decision, error) {
	...
	if m := reJSONFence.FindStringSubmatch(jsonPart); m != nil && len(m) > 1 {
		...
		jsonContent = normalizeDecisionJSON(jsonContent)
...
func normalizeDecisionJSON(jsonStr string) string {
	// Some models emit "reason" instead of the required "reasoning".
	// Accept it here so harmless schema drift does not discard the whole decision.
	return reReasonKey.ReplaceAllString(jsonStr, `"reasoning":`)
}
...
func containsRangeSymbolOutsideStrings(s string) bool {
	inString := false
	escaped := false
