...
export class HttpClient {
  ...
  private async handleError(error: AxiosError): Promise<any> {
    ...
    const response = error.response as AxiosResponse<ErrorPayload | string>
    const { status } = response
    const suppressSystemErrors = Boolean(
      (error.config as any)?.suppressSystemErrors
    )
    ...
    // Handle 429 Rate Limited - upstream throttling or temporary bans
    if (status === 429) {
      if (!suppressSystemErrors) {
  ...
  async request<T = any>(
    ...
    try {
      const response = await this.axiosInstance.request<T>({
        ...
        suppressSystemErrors: options.behavior?.suppressSystemErrors,
      } as any)
      ...
      if (axios.isAxiosError(error) && error.response) {
        ...
        return {
          success: false,
          message: extractErrorMessage(errorData, 'Operation failed'),
        }
      }
  ...
  async get<T = any>(
    ...
    return this.request<T>(url, { method: 'GET', params, headers, behavior })
  }
  ...
  async post<T = any>(
    ...
    return this.request<T>(url, { method: 'POST', data, headers, behavior })
  }
  ...
  async put<T = any>(
    ...
    return this.request<T>(url, { method: 'PUT', data, headers, behavior })
  }
  ...
  async delete<T = any>(
    ...
    return this.request<T>(url, { method: 'DELETE', headers, behavior })
  }
  ...
  async patch<T = any>(
    ...
    return this.request<T>(url, { method: 'PATCH', data, headers, behavior })
  }
}
