...
export const traderApi = {
  ...
  async createShareLink(traderId: string): Promise<{ token: string; url: string }> {
    const result = await httpClient.post<{ token: string; url: string }>(
      `${API_BASE}/traders/${traderId}/share-link`
    )
    if (!result.success || !result.data) throw new Error('Failed to create share link')
    return result.data
  },

  async getPublicShareLink(traderId: string): Promise<{ token: string; url: string }> {
    const result = await httpClient.get<{ token: string; url: string }>(
      `${API_BASE}/traders/${traderId}/public-share-link`
    )
    if (!result.success || !result.data) throw new Error('Failed to fetch public share link')
    return result.data
  },
