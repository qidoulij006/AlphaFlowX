...
export const coinSource = {
  ...
  nofxosNote: { zh: '当前部署已禁用外部量化数据源', en: 'External quant data is disabled in this deployment', es: 'Los datos quant externos están deshabilitados' },
...
export const gridConfig = {
  ...
  totalInvestment: { zh: '初始投入基准 (USDT)', en: 'Initial Base (USDT)', es: 'Base Inicial (USDT)' },
  totalInvestmentDesc: {
    zh: '仅作为网格初始资金与余额异常时的回退基准；当前挂单会按实时账户权益动态复利计算。',
    en: 'Used as the grid seed capital and fallback when balance data is unavailable. Live orders compound dynamically from real-time account equity.',
    es: 'Se usa como capital inicial y respaldo cuando no hay saldo disponible. Las órdenes activas se calculan dinámicamente sobre el equity en tiempo real.'
  },
...
export const gridRisk = {
  ...
  boundaryState: { zh: '边界', en: 'Boundary', es: 'Límite' },
  ...
  compoundingBase: { zh: '复利基准权益', en: 'Compounding Base', es: 'Base de Interés Compuesto' },
  currentPosition: { zh: '当前', en: 'Current', es: 'Actual' },
  maxPosition: { zh: '最大', en: 'Max', es: 'Máximo' },
  positionPercent: { zh: '占比', en: 'Usage', es: 'Uso' },
  longPosition: { zh: '多头库存', en: 'Long Inventory', es: 'Inventario Long' },
  shortPosition: { zh: '空头库存', en: 'Short Inventory', es: 'Inventario Short' },
  inventoryInfo: { zh: '当前库存', en: 'Current Inventory', es: 'Inventario Actual' },
  orderInventory: { zh: '当前挂单', en: 'Open Orders', es: 'Órdenes Abiertas' },
  orderBook: { zh: '挂单盘口', en: 'Order Book', es: 'Libro de Órdenes' },
  avgOrderNotional: { zh: '平均单笔挂单金额', en: 'Avg Order Notional', es: 'Notional Promedio' },
  orderNotional: { zh: '单笔挂单金额', en: 'Order Notional', es: 'Notional' },
  entryOrders: { zh: '入场单', en: 'Entry', es: 'Entrada' },
  reduceOnlyOrders: { zh: '平仓挂单', en: 'Reduce-Only', es: 'Reduce-Only' },
  longOrders: { zh: 'LONG侧', en: 'LONG side', es: 'Lado LONG' },
  shortOrders: { zh: 'SHORT侧', en: 'SHORT side', es: 'Lado SHORT' },
  longLevels: { zh: '多头已成交层数', en: 'Long Filled Levels', es: 'Niveles Long Ejecutados' },
  shortLevels: { zh: '空头已成交层数', en: 'Short Filled Levels', es: 'Niveles Short Ejecutados' },
  longEntryOrders: { zh: '做多挂单', en: 'Long Entry', es: 'Entrada Long' },
  shortEntryOrders: { zh: '做空挂单', en: 'Short Entry', es: 'Entrada Short' },
  longExitOrders: { zh: '多头平仓', en: 'Long Exit', es: 'Salida Long' },
  shortExitOrders: { zh: '空头平仓', en: 'Short Exit', es: 'Salida Short' },
  noOpenOrders: { zh: '当前没有挂单', en: 'No open orders', es: 'Sin órdenes abiertas' },
  ...
  gridBounds: { zh: '当前网格边界', en: 'Current Grid Bounds', es: 'Límites de Grid' },
  boundarySource: { zh: '边界来源', en: 'Boundary Source', es: 'Fuente del Límite' },
  currentSpacing: { zh: '当前网格间隔', en: 'Current Grid Spacing', es: 'Espaciado Actual' },
  minSpacing: { zh: '最小允许间隔', en: 'Minimum Allowed Spacing', es: 'Espaciado Mínimo' },
  maxSpacing: { zh: '最大允许间隔', en: 'Maximum Allowed Spacing', es: 'Espaciado Máximo' },
  boundaryLogic: { zh: '边界逻辑', en: 'Boundary Logic', es: 'Lógica del Límite' },
  boundaryLogicDesc: {
    zh: '先用中期箱体确定主边界，短期箱体只做轻微微调，ATR 只做边缘缓冲；如果计算后的网格间隔过小或过大，再按最小/最大 spacing 规则对边界做二次校正。',
    en: 'Use the mid-term box as the primary boundary, apply only a light short-box micro-adjustment, and use ATR only as an edge buffer. If the resulting grid spacing is too small or too large, the bounds are adjusted again by the min/max spacing rules.',
    es: 'Use la caja media como límite principal, la caja corta como microajuste y ATR solo como buffer.'
  },
  manualBoundary: { zh: '手动设置', en: 'Manual', es: 'Manual' },
  midBoxBoundary: { zh: '中期箱体', en: 'Mid Box', es: 'Caja Media' },
  atrBoundary: { zh: 'ATR 边界', en: 'ATR', es: 'ATR' },
  defaultBoundary: { zh: '默认边界', en: 'Default', es: 'Predeterminado' },
  ...
  compoundingNotice: {
    zh: '当前网格按实时账户总权益复利下单；配置页中的“初始投入基准”仅用于启动时参考和余额读取失败时回退。',
    en: 'This grid compounds from real-time account equity. The configured initial base is used only as startup guidance and as a fallback when balance reads fail.',
    es: 'Este grid compone usando el equity total en tiempo real. La base inicial configurada solo se usa como referencia de arranque y respaldo.'
  },
};
...
export const indicator = {
  ...
  nofxosTitle: { zh: '外部量化数据源', en: 'External Quant Data', es: 'Datos Quant Externos' },
  nofxosDesc: { zh: '外部市场量化数据接口', en: 'External market quant data interface', es: 'Interfaz de datos quant externos' },
  nofxosFeatures: { zh: 'AI500 · OI排行 · 资金流向 · 涨跌榜', en: 'AI500 · OI Ranking · Fund Flow · Price Ranking', es: 'AI500 · OI · NetFlow · Ranking' },
  viewApiDocs: { zh: '外部文档', en: 'External Docs', es: 'Docs Externos' },
  apiKey: { zh: 'API Key', en: 'API Key', es: 'API Key' },
  apiKeyPlaceholder: { zh: '外部数据访问 Key（当前停用）', en: 'External data access key (currently disabled)', es: 'Clave de acceso externa (deshabilitada)' },
  ...
  nofxosDataSources: { zh: '外部量化数据源', en: 'External Quant Sources', es: 'Fuentes Quant Externas' },
  configureApiKey: { zh: '当前部署未启用外部量化数据', en: 'External quant data is disabled in this deployment', es: 'Los datos externos están deshabilitados' },
};
