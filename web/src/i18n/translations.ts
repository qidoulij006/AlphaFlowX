import { replaceBranding } from '../constants/branding'
...
export const translations = {
  en: {
    ...
    // Footer
    footerTitle: 'AlphaFlowX - AI Trading System',
    ...
    heroDescription:
      'AlphaFlowX is the future standard for AI trading — an open, community-driven agentic trading OS. Supporting Binance, Aster DEX and other exchanges, self-hosted, multi-agent competition, let AI automatically make decisions, execute and optimize trades for you.',
    poweredBy: 'Powered by Aster DEX and Binance.',

    // Landing Page CTA
    readyToDefine: 'Ready to define the future of AI trading?',
    startWithCrypto:
      'Starting with crypto markets, expanding to TradFi. AlphaFlowX is the infrastructure of AgentFi.',
    getStartedNow: 'Get Started Now',
    viewSourceCode: 'View Source Code',

    // Features Section
    coreFeatures: 'Core Features',
    whyChooseNofx: 'Why Choose AlphaFlowX?',
    openCommunityDriven:
    ...
    // About Section
    aboutNofx: 'About AlphaFlowX',
    whatIsNofx: 'What is AlphaFlowX?',
    nofxNotAnotherBot:
      "AlphaFlowX is not another trading bot, but the 'Linux' of AI trading —",
    nofxDescription1:
    ...
    howToStart: 'How to Get Started with AlphaFlowX',
    fourSimpleSteps:
      'Four simple steps to start your AI automated trading journey',
    step1Title: 'Run the Installer',
    step1Desc:
      'Run "curl -fsSL https://alphaflowx.com/install.sh | bash" to deploy with the branded installer entrypoint.',
    ...
    riskWarningText:
      'Dev branch is unstable, do not use funds you cannot afford to lose. AlphaFlowX is non-custodial, no official strategies. Trading involves risks, invest carefully.',
    ...
    // Login Modal
    accessNofxPlatform: 'Access AlphaFlowX Platform',
    loginRegisterPrompt:
    ...
    faqSubtitle: 'Find answers to common questions about AlphaFlowX',
    faqStillHaveQuestions: 'Still Have Questions?',
    faqContactUs: 'Join our community for more help',
    ...
    // ===== GETTING STARTED =====
    faqWhatIsNOFX: 'What is AlphaFlowX?',
    faqWhatIsNOFXAnswer:
      'AlphaFlowX is an AI-powered trading operating system for cryptocurrency and US stock markets. It uses large language models (LLMs) like DeepSeek, GPT, Claude, Gemini to analyze market data and make autonomous trading decisions. Key features include: multi-AI model support, multi-exchange trading, and visual strategy builder.',

    faqHowDoesItWork: 'How does AlphaFlowX work?',
    faqHowDoesItWorkAnswer:
      'AlphaFlowX works in 5 steps: 1) Configure AI models and exchange API credentials; 2) Create a trading strategy (coin selection, indicators, risk controls); 3) Create a "Trader" combining AI model + Exchange + Strategy; 4) Start the trader - it will analyze market data at regular intervals and make buy/sell/hold decisions; 5) Monitor performance on the dashboard. The AI uses Chain of Thought reasoning to explain each decision.',

    faqIsProfitable: 'Is AlphaFlowX profitable?',
    faqIsProfitableAnswer:
      'AI trading is experimental and NOT guaranteed to be profitable. Cryptocurrency futures are highly volatile and risky. AlphaFlowX is designed for educational and research purposes. We strongly recommend: starting with small amounts (10-50 USDT), never investing more than you can afford to lose, thoroughly testing before live trading, and understanding that past performance does not guarantee future results.',
    ...
    // ===== INSTALLATION =====
    faqHowToInstall: 'How do I install AlphaFlowX?',
    faqHowToInstallAnswer:
      'Easiest method (Linux/macOS): Run "curl -fsSL https://alphaflowx.com/install.sh | bash" - this installs Docker containers automatically. Then open http://127.0.0.1:3000 in your browser. For manual installation or development, clone the code repository and follow the README instructions.',
    ...
    faqUpdateNOFX: 'How do I update AlphaFlowX?',
    faqUpdateNOFXAnswer:
    ...
    faqStopLossTakeProfit: 'Does AlphaFlowX support stop-loss and take-profit?',
    ...
    faqMultipleTradersAnswer:
      'Yes! AlphaFlowX supports running 20+ concurrent traders. Each trader can have different: AI model, exchange account, strategy, decision interval. Use this to A/B test strategies, compare AI models, or diversify across exchanges. Monitor all traders on the Competition page.',
    ...
    faqBinancePositionModeAnswer:
      'Error: "Order\'s position side does not match user\'s setting". You\'re in One-way Mode but AlphaFlowX requires Hedge Mode. Fix: 1) Close ALL positions first; 2) Binance Futures → Settings (gear icon) → Preferences → Position Mode → Switch to "Hedge Mode" (双向持仓); 3) Restart your trader.',
    ...
    faqEncryptionDetailsAnswer:
      'AlphaFlowX uses multiple encryption layers: 1) AES-256-GCM for database storage (API keys, secrets); 2) RSA-2048 for optional transport encryption (browser to server); 3) JWT for authentication tokens. Keys are generated during installation. Enable TRANSPORT_ENCRYPTION=true for HTTPS environments.',

    faqSecurityBestPractices: 'What are security best practices?',
    faqSecurityBestPracticesAnswer:
      'Recommended: 1) Use exchange API keys with IP whitelist and minimal permissions (Futures Trading only); 2) Use a dedicated subaccount for AlphaFlowX; 3) Enable TRANSPORT_ENCRYPTION for remote deployments; 4) Never share .env or data.db files; 5) Use HTTPS with valid certificates; 6) Regularly rotate API keys; 7) Monitor account activity.',

    faqCanNOFXStealFunds: 'Can AlphaFlowX steal my funds?',
    faqCanNOFXStealFundsAnswer:
      'API keys are stored locally on YOUR machine, never sent to external servers. AlphaFlowX only has the permissions you grant via API keys. For maximum safety: use API keys with trading-only permissions (no withdrawal), enable IP whitelist, use a dedicated subaccount.',
  ...
  zh: {
    ...
    step1Desc:
      '运行 "curl -fsSL https://alphaflowx.com/install.sh | bash" 使用品牌化安装入口完成部署。',
    step2Title: '配置环境',
    step2Desc:
    ...
    faqWhatIsNOFXAnswer:
      'NOFX 是一个 AI 驱动交易操作系统，支持加密货币和美股市场。它使用大语言模型（LLM）如 DeepSeek、GPT、Claude、Gemini 来分析市场数据，进行自主交易决策。核心功能包括：多 AI 模型支持、多交易所交易、可视化策略构建器、回测系统。',
    ...
    faqHowToInstallAnswer:
      '最简单的方法（Linux/macOS）：运行 "curl -fsSL https://alphaflowx.com/install.sh | bash" - 这会自动安装 Docker 容器。然后在浏览器中打开 http://127.0.0.1:3000。手动安装或开发请克隆代码仓库并按照 README 说明操作。',
    ...
    faqManualInstallationAnswer:
      '前置条件：Go 1.21+、Node.js 18+、TA-Lib。步骤：1）克隆代码仓库："git clone https://github.com/NoFxAiOS/nofx.git"；2）安装后端依赖："go mod download"；3）安装前端依赖："cd web && npm install"；4）构建后端："go build -o nofx"；5）运行后端："./nofx"；6）运行前端（新终端）："cd web && npm run dev"。访问 http://127.0.0.1:3000',
    ...
    faqCanNOFXStealFundsAnswer:
      'API 密钥存储在您的机器本地，从不发送到外部服务器。NOFX 只有您通过 API 密钥授予的权限。为最大安全：使用仅交易权限（无提现）的 API 密钥，启用 IP 白名单，使用专用子账户。',
  ...
  id: {
    ...
    step1Desc: 'Jalankan "curl -fsSL https://alphaflowx.com/install.sh | bash" untuk deploy melalui entrypoint installer bermerek.',
    ...
    faqHowToInstallAnswer: 'Metode termudah (Linux/macOS): Jalankan "curl -fsSL https://alphaflowx.com/install.sh | bash" - ini menginstal kontainer Docker secara otomatis. Lalu buka http://127.0.0.1:3000 di browser Anda.',
