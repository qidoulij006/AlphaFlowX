...
export const HIDDEN_MODEL_IDS = new Set(['claw402', 'blockrun-base', 'blockrun-sol'])
export const HIDDEN_MODEL_PROVIDERS = new Set(['claw402'])

export function isHiddenModel(model: { id?: string; provider?: string } | string): boolean {
  if (typeof model === 'string') {
    const normalized = model.toLowerCase()
    return HIDDEN_MODEL_IDS.has(normalized) || HIDDEN_MODEL_PROVIDERS.has(normalized) || normalized.startsWith('blockrun')
  }
  return (
    (model.id ? HIDDEN_MODEL_IDS.has(model.id.toLowerCase()) || model.id.toLowerCase().startsWith('blockrun') : false) ||
    (model.provider ? HIDDEN_MODEL_PROVIDERS.has(model.provider.toLowerCase()) || model.provider.toLowerCase().startsWith('blockrun') : false)
  )
}

export function filterVisibleModels<T extends { id?: string; provider?: string }>(models: T[] | undefined | null): T[] {
  return (models || []).filter((model) => !isHiddenModel(model))
}
