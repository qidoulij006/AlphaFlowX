...
import { GridRiskPanel } from '../strategy/GridRiskPanel'

// 提取下划线后面的名称部分
function getShortName(fullName: string): string {
  if (fullName.toLowerCase().includes('claw402')) return 'private'
...
const EXCHANGE_REGISTRATION_LINKS: Record<string, { url: string; hasReferral?: boolean }> = {
  binance: { url: 'https://www.binance.com/join?ref=AFXENG', hasReferral: true },
...
export function TraderConfigModal({
  ...
  const isGridStrategy =
    selectedStrategy?.config?.strategy_type === 'grid_trading' ||
    selectedStrategy?.name?.toLowerCase().includes('grid') ||
    selectedStrategy?.name?.includes('网格')
  const gridInitialBase =
    typeof selectedStrategy?.config?.grid_config?.total_investment === 'number'
      ? selectedStrategy.config.grid_config.total_investment
      : 0

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
      <div
        className="rounded-xl shadow-2xl max-w-2xl w-full my-8"
        style={{ maxHeight: 'calc(100vh - 4rem)', background: 'linear-gradient(180deg, var(--panel-bg) 0%, color-mix(in srgb, var(--panel-bg-solid) 92%, black) 100%)', border: '1px solid var(--panel-border)' }}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-6 sticky top-0 z-10 rounded-t-xl" style={{ borderBottom: '1px solid var(--panel-border)', background: 'linear-gradient(to right, var(--panel-bg), color-mix(in srgb, var(--panel-bg-solid) 86%, black))' }}>
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-lg text-nofx-text-muted hover:text-nofx-text-main transition-colors flex items-center justify-center"
        ...
        >
          {/* Basic Info */}
          <div className="bg-[var(--panel-bg-solid)] border border-[var(--panel-border)] rounded-lg p-5">
            <h3 className="text-lg font-semibold text-nofx-text-main mb-5 flex items-center gap-2">
              <span className="text-[#F0B90B]">1</span> {t('basicConfig', language)}
            </h3>
            <div className="space-y-4">
              <div>
                <label className="text-sm text-nofx-text-main block mb-2">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-[var(--panel-bg)] border border-[var(--panel-border)] rounded text-nofx-text-main focus:border-[#F0B90B] focus:outline-none"
                  ...
                  {formData.exchange_id && (() => {
                    ...
                    return (
                      <a
                        ...
                        className="mt-2 inline-flex items-center gap-1.5 text-xs text-nofx-text-muted hover:text-[#F0B90B] transition-colors"
                      >
          ...
          <div className="bg-[var(--panel-bg-solid)] border border-[var(--panel-border)] rounded-lg p-5">
            ...
            <div className="space-y-4">
              ...
              {/* Strategy Preview */}
              {selectedStrategy && (
                <div className="mt-3 p-4 bg-[var(--panel-bg)] border border-[var(--panel-border)] rounded-lg">
                  ...
                  {isGridStrategy && (
                    <div className="mt-3 rounded-md border border-[#F0B90B]/20 bg-[#F0B90B]/5 px-3 py-2 text-xs leading-5 text-nofx-text-muted">
                      <div className="text-nofx-text-main">
                        {language === 'zh' ? '网格初始基准' : 'Grid Initial Base'}: {gridInitialBase > 0 ? `${gridInitialBase.toLocaleString('en-US', { maximumFractionDigits: 0 })} USDT` : '-'}
                      </div>
                      <div>
                        {language === 'zh'
                          ? '该策略当前会按实时账户总权益复利下单；这里的金额主要用于启动参考和余额读取失败时回退。'
                          : 'This strategy currently compounds from real-time total account equity. The configured amount is mainly used as startup guidance and fallback when balance reads fail.'}
                      </div>
                    </div>
                  )}
                </div>
          ...
          {isEditMode && traderData?.trader_id && isGridStrategy && (
            <div className="bg-[var(--panel-bg-solid)] border border-[var(--panel-border)] rounded-lg p-5">
              <h3 className="text-lg font-semibold text-nofx-text-main mb-5 flex items-center gap-2">
                <span className="text-[#F0B90B]">3</span> Grid Runtime
              </h3>
              <GridRiskPanel
          ...
          <div className="bg-[var(--panel-bg-solid)] border border-[var(--panel-border)] rounded-lg p-5">
            <h3 className="text-lg font-semibold text-nofx-text-main mb-5 flex items-center gap-2">
              <span className="text-[#F0B90B]">{isEditMode && traderData?.trader_id && isGridStrategy ? '4' : '3'}</span> {t('tradingParams', language)}
            </h3>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <button
                      ...
                      className={`flex-1 px-3 py-2 rounded text-sm ${
                        formData.is_cross_margin
                          ...
                          : 'bg-[var(--panel-bg)] text-nofx-text-muted border border-[var(--panel-border)]'
                      }`}
                    >
              ...
              {isEditMode && (
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-sm text-nofx-text-main">
                    ...
                    <button
                      ...
                      className="px-3 py-1 text-xs bg-[#F0B90B] text-black rounded hover:bg-[#E1A706] transition-colors disabled:cursor-not-allowed"
                      style={{ opacity: isFetchingBalance ? 0.7 : 1 }}
                    >
              ...
              {/* Create mode info */}
              {!isEditMode && (
                <div className="p-3 bg-[var(--panel-bg)] border border-[var(--panel-border)] rounded flex items-center gap-2">
                  <svg
        ...
        {/* Footer */}
        <div className="flex justify-end gap-3 p-6 sticky bottom-0 z-10 rounded-b-xl" style={{ borderTop: '1px solid var(--panel-border)', background: 'linear-gradient(to right, var(--panel-bg), color-mix(in srgb, var(--panel-bg-solid) 86%, black))' }}>
          <button
            onClick={onClose}
            className="px-6 py-3 bg-[var(--panel-border)] text-nofx-text-main rounded-lg hover:bg-[var(--panel-bg-hover)] transition-all duration-200 border border-[var(--panel-border)]"
          >
            {t('cancel', language)}
          </button>
          {onSave && (
            <button
              ...
              className="px-8 py-3 bg-gradient-to-r from-[#F0B90B] to-[#E1A706] text-black rounded-lg hover:from-[#E1A706] hover:to-[#D4951E] transition-all duration-200 disabled:cursor-not-allowed font-medium shadow-lg"
              style={{ opacity: isSaving || !formData.trader_name || !formData.ai_model || !formData.exchange_id ? 0.7 : 1 }}
            >
