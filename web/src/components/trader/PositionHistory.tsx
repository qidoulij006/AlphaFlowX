...
export function PositionHistory({ traderId, shareToken }: PositionHistoryProps) {
  ...
  useEffect(() => {
    const fetchData = async (showLoading: boolean = true) => {
      try {
        ...
        const data = await (shareToken
          ? api.getSharedPositionHistory(shareToken, Math.max(200, pageSize * 5))
      ...
      >
            ...
            <div className="flex rounded overflow-hidden" style={{ border: '1px solid var(--panel-border)' }}>
              {['all', 'LONG', 'SHORT'].map((side) => (
                <button
                  ...
                  style={{
                    background: filterSide === side ? 'var(--panel-border)' : 'transparent',
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr style={{ background: 'var(--panel-bg-solid)' }}>
                <th
            ...
            {totalPages > 1 && (
              <div className="flex items-center gap-1">
                <button
                  ...
                  style={{
                    background: currentPage === 1 ? 'transparent' : 'var(--panel-border)',
                    color: 'var(--text-primary)',
                  }}
                >
                  «
                ...
                <button
                  ...
                  className="px-2 py-1 rounded text-xs transition-colors disabled:opacity-30"
                  style={{
                    background: currentPage === totalPages ? 'transparent' : 'var(--panel-border)',
                    color: 'var(--text-primary)',
                  }}
                >
                  ›
