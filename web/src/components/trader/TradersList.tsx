...
function TraderRow({
  ...
  const traderIssue = getTraderIssueText(trader, language)

  return (
    <div
      className="flex flex-col p-3 md:p-4 rounded transition-all hover:translate-y-[-1px] gap-3 md:gap-4"
      style={{ background: 'var(--panel-bg)', border: '1px solid var(--panel-border)' }}
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-4">
        ...
        <div className="flex items-center gap-3 md:gap-4 flex-wrap md:flex-nowrap">
          {isPerpDex && walletAddr && (
            ...
            >
              <span className="text-xs font-mono font-medium" style={{ color: '#F0B90B' }}>
            ...
            <button
              ...
              style={{
                background: trader.is_running
                  ...
                  : 'rgba(255, 193, 7, 0.1)',
                color: trader.is_running ? 'var(--text-secondary)' : '#FFC107',
              }}
            >
...
function getTraderIssueText(trader: TraderInfo, language: Language): string | null {
  switch (trader.error_hint_code) {
    case 'okx_account_mode_mismatch':
      return language === 'zh'
        ? 'OKX 账户模式不兼容。请检查双向持仓(Hedge Mode)、不要使用仅支持 net mode 的 Portfolio Margin，并确认合约保证金模式为 Cross。'
        : 'OKX account mode is incompatible. Check Hedge Mode, avoid Portfolio Margin that only supports net mode, and use Cross margin for swaps.'
    case 'binance_position_mode_mismatch':
      return language === 'zh'
        ? '交易所持仓模式与系统下单方向不匹配。请切换到双向持仓(Hedge Mode)。'
        : 'The exchange position mode does not match the order side. Switch the account to Hedge Mode.'
    default:
      return trader.error_hint || trader.error_message || null
  }
}
