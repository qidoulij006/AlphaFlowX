...
export function DeepVoidBackground({
    ...
    return (
        <div className={`deepvoid-shell relative w-full min-h-screen bg-nofx-bg text-nofx-text overflow-hidden flex flex-col ${className}`} {...props}>
            ...
            <div className="deepvoid-noise absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20 mix-blend-soft-light pointer-events-none fixed z-0"></div>

            {/* 2. Grid System */}
            <div className="deepvoid-grid absolute inset-0 pointer-events-none fixed z-0">
                <div className="absolute inset-x-0 bottom-0 h-[50vh] bg-[linear-gradient(to_right,var(--panel-border)_1px,transparent_1px),linear-gradient(to_bottom,var(--panel-border)_1px,transparent_1px)] bg-[size:40px_40px] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)] opacity-50" style={{ transform: 'perspective(500px) rotateX(60deg) translateY(100px) scale(2)' }}></div>
                <div className="absolute inset-0 bg-grid-pattern opacity-[0.03]"></div>
            </div>

            {/* 3. Ambient Glow Spots */}
            <div className="deepvoid-glow absolute inset-0 overflow-hidden pointer-events-none fixed z-0">
            ...
            <div className="deepvoid-scanlines absolute inset-0 pointer-events-none fixed z-[9999] opacity-40">
