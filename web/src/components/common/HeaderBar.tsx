  ...
  return (
    <nav className="fixed top-0 w-full z-50 header-bar">
      <div className="flex items-center justify-between h-16 px-4 sm:px-6 max-w-[1920px] mx-auto">
        ...
        >
          <img src={BRAND_INFO.iconPath} alt={`${BRAND_INFO.name} Logo`} className="w-7 h-7" />
        ...
        <div className="hidden md:flex items-center justify-between flex-1 ml-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                type="button"
                className="p-2 rounded-lg text-nofx-text-muted opacity-60 cursor-default"
                title="GitHub"
                aria-disabled="true"
              >
            ...
            <ThemeToggle compact className="hidden xl:flex" />
            ...
            {isLoggedIn && user ? (
                  ...
                  {userDropdownOpen && (
                    <div className="absolute right-0 top-full mt-2 w-48 rounded-lg shadow-lg overflow-hidden z-50 bg-nofx-bg-lighter border border-nofx-gold/20">
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-2 px-3 py-2 text-sm transition-colors hover:bg-white/5 text-nofx-text-muted hover:text-nofx-text-main"
                      >
              ...
              currentPage !== 'register' && (
                <div className="flex items-center gap-3">
                  <a
                    href="/login"
                    className="px-3 py-2 text-sm font-medium transition-colors rounded text-nofx-text-muted hover:text-nofx-text-main"
                  >
                    {t('signIn', language)}
                  </a>
                </div>
              ...
              {languageDropdownOpen && (
                <div className="absolute right-0 top-full mt-2 w-32 rounded-lg shadow-lg overflow-hidden z-50 bg-nofx-bg-lighter border border-nofx-gold/20">
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-2 transition-colors text-nofx-text-muted hover:text-nofx-text-main
        ...
        <motion.button
          ...
          className="md:hidden text-nofx-text-muted hover:text-nofx-text-main"
          whileTap={{ scale: 0.9 }}
        >
          {mobileMenuOpen ? (
            <X className="w-6 h-6" />
            ...
            >
                  ...
                  return navTabs.map((tab, i) => (
                    <motion.button
                      ...
                      className={`text-2xl font-black tracking-tight text-left flex items-center gap-3
                        ${currentPage === tab.page ? 'text-nofx-gold' : 'text-nofx-text-muted'}`}
                    >
                      {currentPage === tab.page && (
                        <motion.div
                      ...
                      {tab.requiresAuth && !isLoggedIn && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded border border-[var(--panel-border)] text-nofx-text-muted font-normal tracking-wide uppercase align-middle relative -top-1">
                          LOGIN_REQ
                        </span>
                      )}
                    </motion.button>
                  ...
                  <div className="pt-6 border-t border-white/5 space-y-4">
                    {[
                      ...
                      <motion.a
                        ...
                        className="block text-lg font-mono text-nofx-text-muted hover:text-nofx-text-main"
              ...
              <div className="mt-auto space-y-8">
                ...
                <div className="flex items-center gap-4">
                  {[
                    ...
                    <button
                      type="button"
                      key={i}
                      className="w-12 h-12 rounded-full bg-[var(--panel-bg-solid)] border border-[var(--panel-border)] flex items-center justify-center text-nofx-text-muted opacity-60 cursor-default"
                      aria-disabled="true"
                    >
                ...
                <div className="grid grid-cols-1 gap-4">
                  {/* Lang Switcher */}
                  <div className="flex bg-[var(--panel-bg-solid)] rounded-lg p-1 border border-[var(--panel-border)]">
                    {['zh', 'en'].map((lang) => (
                      <button
                        key={lang}
                        onClick={() => {
                        ...
                        className={`flex-1 py-3 text-sm font-bold rounded-md transition-colors ${language === lang
                          ? 'bg-[var(--background)] text-nofx-text-main shadow-sm'
                  ...
                  <ThemeToggle compact className="justify-center" />
