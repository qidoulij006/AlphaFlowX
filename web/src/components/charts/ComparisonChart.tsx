  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <div className="text-sm mt-4 font-medium" style={{ color: 'var(--text-secondary)' }}>
  ...
  if (combinedData.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <div className="text-sm text-center max-w-xs" style={{ color: 'var(--text-secondary)' }}>
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        ...
        >
          <div className="flex items-center gap-2 mb-3 pb-2" style={{ borderBottom: '1px solid var(--panel-border)' }}>
              ...
              return (
                <div key={trader.trader_id} className="flex items-center justify-between gap-4">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-[10px] mono" style={{ color: 'var(--text-secondary)' }}>
                      ${equity?.toFixed(2)}
                    </div>
                  </div>
                </div>
  ...
  return (
    <div className="space-y-4">
            ...
            <button
              ...
              style={{
                background: selectedPeriod === period.key
                  ? 'rgba(240, 185, 11, 0.2)'
                  : 'color-mix(in srgb, var(--panel-border) 45%, transparent)',
                color: selectedPeriod === period.key ? '#F0B90B' : 'var(--text-secondary)',
                border: `1px solid ${selectedPeriod === period.key ? 'rgba(240, 185, 11, 0.4)' : 'var(--panel-border)'}`,
              }}
            >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {traderStats.slice(0, 3).map((trader, idx) => (
            <div key={trader.trader_id}
                 ...
                 style={{
                   background: idx === 0 ? 'rgba(240, 185, 11, 0.15)' : 'color-mix(in srgb, var(--panel-border) 45%, transparent)',
                   border: `1px solid ${idx === 0 ? 'rgba(240, 185, 11, 0.3)' : 'var(--panel-border)'}`
                 }}>
      ...
      <div className="relative rounded-xl overflow-hidden"
           style={{ background: 'linear-gradient(180deg, color-mix(in srgb, var(--panel-bg) 86%, transparent) 0%, var(--panel-bg) 100%)', border: '1px solid var(--panel-border)' }}>
        {/* Watermark */}
        <div style={{
          position: 'absolute',
          top: '50%',
        ...
        <ResponsiveContainer width="100%" height={420}>
          ...
          >
            ...
            <CartesianGrid strokeDasharray="3 3" stroke="color-mix(in srgb, var(--panel-border) 70%, transparent)" vertical={false} />
