...
const inferOrderReduceOnly = (order: OpenOrder): boolean => {
  if (typeof order.reduce_only === 'boolean') return order.reduce_only
  const side = order.side?.toUpperCase()
  const positionSide = order.position_side?.toUpperCase()
  return (side === 'SELL' && positionSide === 'LONG') || (side === 'BUY' && positionSide === 'SHORT')
}

const formatOrderPriceLabel = (price: number): string => {
...
const getOpenOrderBucket = (order: OpenOrder): string => {
  const type = order.type?.toUpperCase() || ''
  const isReduceOnly = inferOrderReduceOnly(order)
  const positionSide = order.position_side?.toUpperCase() || ''
  if (type.includes('STOP') || type.includes('SL')) return 'stop_loss'
  if (type.includes('TAKE_PROFIT') || type.includes('TP')) return 'take_profit'
  if (isReduceOnly && positionSide === 'LONG') return 'long_exit'
  if (isReduceOnly && positionSide === 'SHORT') return 'short_exit'
  if (positionSide === 'LONG') return 'long_entry'
  return 'short_entry'
}

const aggregateOpenOrders = (orders: OpenOrder[], language: string): AggregatedOpenOrder[] => {
  const grouped = new Map<string, AggregatedOpenOrder>()

  orders.forEach((order) => {
    const linePrice = order.stop_price > 0 ? order.stop_price : order.price
    if (linePrice <= 0) return

    const bucket = getOpenOrderBucket(order)
    const priceLabel = formatOrderPriceLabel(linePrice)
    const key = `${bucket}|${priceLabel}`
    ...
    switch (bucket) {
      case 'stop_loss':
        color = '#F6465D'
        title = language === 'zh' ? '止损单' : 'Stop Loss'
        break
      case 'take_profit':
        color = '#0ECB81'
        title = language === 'zh' ? '止盈单' : 'Take Profit'
        break
      case 'long_exit':
        color = '#22C55E'
        title = language === 'zh' ? '多头平仓' : 'Long Exit'
        break
      case 'short_exit':
        color = '#FB7185'
        title = language === 'zh' ? '空头平仓' : 'Short Exit'
        break
      case 'long_entry':
        color = '#0ECB81'
        title = language === 'zh' ? '做多挂单' : 'Long Entry'
        break
      default:
        color = '#F6465D'
        title = language === 'zh' ? '做空挂单' : 'Short Entry'
        break
    }
  ...
  return Array.from(grouped.values()).sort((a, b) => {
    if (a.bucket === b.bucket) return b.linePrice - a.linePrice
...
export function AdvancedChart({
  ...
  const boundaryLinesRef = useRef<any[]>([]) // Store grid / box overlay lines
  ...
  const fetchOrders = async (traderID: string, symbol: string): Promise<OrderMarker[]> => {
    try {
      ...
      const result = await httpClient.get(
        shareToken
          ? `/api/shared/orders?token=${encodeURIComponent(shareToken)}&symbol=${symbol}&status=FILLED&limit=200`
  ...
  const fetchOpenOrders = async (traderID: string, symbol: string): Promise<OpenOrder[]> => {
    try {
      ...
      const result = await httpClient.get(
        shareToken
          ? `/api/shared/open-orders?token=${encodeURIComponent(shareToken)}&symbol=${symbol}`
  ...
  const fetchGridRiskOverlay = async (traderID: string): Promise<GridRiskOverlay | null> => {
    try {
      const result = await httpClient.get(
        shareToken
      ...
      return result.data as GridRiskOverlay
    } catch (err) {
      console.error('[AdvancedChart] Error fetching grid risk overlay:', err)
      return null
    }
  }
  ...
  useEffect(() => {
    ...
    const chart = createChart(chartContainerRef.current, {
      ...
      layout: {
        background: { color: getThemeColor('--panel-bg', '#0B0E11') },
        textColor: getThemeColor('--text-secondary', '#B7BDC6'),
        fontSize: 12,
      },
      grid: {
        vertLines: {
      ...
      rightPriceScale: {
        borderColor: getThemeColor('--panel-border', '#2B3139'),
        scaleMargins: {
          top: 0.1,
          bottom: 0.25,
        },
    ...
    const loadData = async (isRefresh = false) => {
      ...
      try {
        ...
        if (klineData.length > 0) {
          ...
          const precision = getPricePrecision(latestPrice)
  ...
  useEffect(() => {
    ...
    const loadOpenOrders = async () => {
      try {
        ...
        const aggregatedOpenOrders = aggregateOpenOrders(openOrders, language)
        console.log('[AdvancedChart] Open orders for price lines:', aggregatedOpenOrders)

        if (aggregatedOpenOrders.length > 0 && candlestickSeriesRef.current) {
          aggregatedOpenOrders.forEach(order => {
            const priceLine = candlestickSeriesRef.current?.createPriceLine({
              price: order.linePrice,
              color: order.color,
              lineWidth: 1,
              lineStyle: 2,
              axisLabelVisible: true,
              title: `${order.title} ${order.quantity}${order.orderCount > 1 ? ` ×${order.orderCount}` : ''}`,
            })
  ...
  }, [symbol, traderID, language, shareToken])
  ...
  useEffect(() => {
    ...
    const loadGridOverlay = async () => {
      try {
        ...
        const overlay = await fetchGridRiskOverlay(traderID)
        if (!overlay || !candlestickSeriesRef.current) return

        const lines = [
          {
            price: overlay.grid_upper_price,
            color: '#F0B90B',
            title: language === 'zh' ? '网格上边界' : 'Grid Upper',
            lineWidth: 2 as const,
          },
          {
            price: overlay.grid_lower_price,
            color: '#F0B90B',
            title: language === 'zh' ? '网格下边界' : 'Grid Lower',
            lineWidth: 2 as const,
          },
          {
            price: overlay.mid_box_upper,
            color: '#60A5FA',
            title: language === 'zh' ? '中期箱体上沿' : 'Mid Box Upper',
            lineWidth: 1 as const,
          },
          {
            price: overlay.mid_box_lower,
            color: '#60A5FA',
            title: language === 'zh' ? '中期箱体下沿' : 'Mid Box Lower',
            lineWidth: 1 as const,
          },
          {
            price: overlay.short_first_entry_guard_price,
            color: '#F97316',
            title: language === 'zh'
              ? `空头首仓保护线 (${overlay.first_entry_guard_active_source || 'guard'})`
              : `Short First Entry Guard (${overlay.first_entry_guard_active_source || 'guard'})`,
            lineWidth: 1 as const,
          },
          {
            price: overlay.long_first_entry_guard_price,
            color: '#22C55E',
            title: language === 'zh'
              ? `多头首仓保护线 (${overlay.first_entry_guard_active_source || 'guard'})`
              : `Long First Entry Guard (${overlay.first_entry_guard_active_source || 'guard'})`,
            lineWidth: 1 as const,
          },
          {
            price: overlay.current_short_position > 0 ? overlay.short_reference_stop_price : 0,
            color: '#DC2626',
            title: language === 'zh' ? '空头参考止损线' : 'Short Ref Stop',
            lineWidth: 2 as const,
          },
          {
            price: overlay.current_long_position > 0 ? overlay.long_reference_stop_price : 0,
            color: '#BE123C',
            title: language === 'zh' ? '多头参考止损线' : 'Long Ref Stop',
            lineWidth: 2 as const,
          },
        ...
        lines.forEach((line) => {
          ...
          if (priceLine) boundaryLinesRef.current.push(priceLine)
        })
      } catch (err) {
        console.error('[AdvancedChart] Error loading grid overlay:', err)
      }
    }

    const initialTimeout = setTimeout(loadGridOverlay, 1200)
    const overlayInterval = setInterval(loadGridOverlay, 30000)

    return () => {
      clearTimeout(initialTimeout)
      clearInterval(overlayInterval)
    }
  }, [traderID, symbol, language, shareToken])
  ...
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(180deg, color-mix(in srgb, var(--panel-bg-solid) 88%, black) 0%, var(--panel-bg) 100%)',
        borderRadius: '12px',
        overflow: 'hidden',
    ...
    >
      ...
      <div
        ...
        style={{ borderBottom: '1px solid color-mix(in srgb, var(--panel-border) 90%, transparent)', background: 'color-mix(in srgb, var(--panel-bg-solid) 94%, black)', flexShrink: 0 }}
      >
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              ...
              style={{ background: 'color-mix(in srgb, var(--panel-border) 60%, transparent)', color: 'var(--text-secondary)' }}
            >
              {interval}
            </span>
            <span
          ...
          {/* Price Display */}
          {marketStats && (
            <div className="flex items-center gap-3 pl-3" style={{ borderLeft: '1px solid var(--panel-border)' }}>
              <span
                className="text-base font-bold tabular-nums"
                style={{ color: marketStats.priceChange >= 0 ? '#10B981' : '#EF4444' }}
              >
                {marketStats.price.toLocaleString(undefined, {
                  minimumFractionDigits: getPricePrecision(marketStats.price),
                  maximumFractionDigits: exchange === 'forex' || exchange === 'metals'
                    ? Math.max(4, getPricePrecision(marketStats.price))
                    : getPricePrecision(marketStats.price)
                })}
              </span>
              <span
              ...
              <div className="flex items-center gap-2 text-[11px]" style={{ color: 'var(--text-secondary)' }}>
                <span>H <span style={{ color: 'var(--text-primary)' }}>{marketStats.high.toFixed(2)}</span></span>
                <span>L <span style={{ color: 'var(--text-primary)' }}>{marketStats.low.toFixed(2)}</span></span>
      ...
      {showIndicatorPanel && (
        <div
          ...
          style={{
            background: 'linear-gradient(135deg, color-mix(in srgb, var(--panel-bg-solid) 92%, black) 0%, color-mix(in srgb, var(--panel-bg) 96%, black) 100%)',
        ...
        >
          ...
          <div className="p-3 space-y-1">
            {indicators.map(indicator => (
              <label
                key={indicator.id}
                className="flex items-center gap-3 p-2.5 rounded-md cursor-pointer transition-all group"
              ...
              >
                ...
                <div
                  ...
                  style={{ border: '1px solid color-mix(in srgb, var(--text-primary) 10%, transparent)', backgroundColor: indicator.color }}
                ></div>
                <span className="text-sm transition-colors flex-1" style={{ color: 'var(--text-secondary)' }}>
                  {indicator.name}
                </span>
                {indicator.enabled && (
          ...
          <div
            ...
            style={{ borderColor: 'rgba(43, 49, 57, 0.5)', color: 'var(--text-secondary)' }}
          >
        ...
        {tooltipData && (
          <div
            ...
            style={{
              ...
              top: '10px',
              padding: '8px 12px',
              background: 'color-mix(in srgb, var(--panel-bg-solid) 94%, black)',
