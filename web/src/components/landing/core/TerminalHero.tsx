    ...
    return (
        <section className="relative w-full min-h-screen bg-nofx-bg text-nofx-text overflow-hidden flex flex-col pt-20">
            ...
            <div className="relative z-10 flex-1 grid grid-cols-1 lg:grid-cols-12 gap-0 lg:gap-8 max-w-[1800px] mx-auto w-full px-6 h-full pb-20 pt-10 pointer-events-none">
                ...
                <div className="hidden lg:flex col-span-3 flex-col justify-between h-full border-r border-[var(--glass-border)] pr-8 py-10 pointer-events-auto">

                    {/* Top: System Health */}
                    <div className="space-y-6">
                        <div className="tech-border p-4 bg-[var(--panel-bg)]/80 backdrop-blur-sm">
                            <h3 className="text-xs font-mono text-nofx-gold mb-4 flex items-center gap-2">
                                <Activity className="w-3 h-3" /> {language === 'zh' ? '系统诊断' : 'SYSTEM_DIAGNOSTICS'}
                            </h3>
                            <div className="space-y-3 font-mono text-[10px] text-nofx-text-muted">
                                <div className="flex justify-between items-center">
                                    <span>{language === 'zh' ? '核心延迟' : 'KERNEL_LATENCY'}</span>
                                ...
                                <div className="flex justify-between items-center">
                                    <span>{language === 'zh' ? '内存完整性' : 'MEMORY_INTEGRITY'}</span>
                                ...
                                <div className="flex justify-between items-center">
                                    <span>{language === 'zh' ? '在线率' : 'UPTIME'}</span>
                                    <span className="text-nofx-text-main">99.999%</span>
                                </div>
                            </div>
                        </div>

                        <div className="p-4 border border-[var(--glass-border)] rounded bg-[var(--panel-bg)]/50">
                            <div className="flex items-center gap-3 text-nofx-text-muted mb-2">
                                <Shield className="w-4 h-4" />
                                <span className="text-[10px] font-mono tracking-widest">{language === 'zh' ? '安全协议' : 'SECURITY PROTOCOLS'}</span>
                            </div>
                            <div className="flex gap-1">
                                <div className="h-1 flex-1 bg-nofx-gold"></div>
                                <div className="h-1 flex-1 bg-nofx-gold"></div>
                                <div className="h-1 flex-1 bg-nofx-gold"></div>
                                <div className="h-1 flex-1 bg-[var(--glass-border)]"></div>
                            </div>
                            <div className="mt-2 text-right text-[10px] text-nofx-gold/80 font-mono">{language === 'zh' ? '三级防护已启用' : 'LEVEL 3 ACTIVE'}</div>
                        </div>
                    </div>

                    {/* Bottom: Network Log */}
                    <div className="font-mono text-[10px] text-[var(--text-tertiary)] space-y-1 opacity-70">
                        <div>{language === 'zh' ? '> 正在连接主节点... 完成' : '> CONNECTING TO MAINNET... OK'}</div>
                        <div>{language === 'zh' ? '> 正在同步节点 (424/424)... 完成' : '> SYNCING NODES (424/424)... OK'}</div>
                        <div>{language === 'zh' ? '> 正在加载资产... 完成' : '> LOADING ASSETS... DONE'}</div>
                        <div className="animate-pulse">{language === 'zh' ? '> 等待用户指令_' : '> AWAITING USER INPUT_'}</div>
                    </div>
                </div>
                ...
                <div className="col-span-1 lg:col-span-6 flex flex-col items-center justify-center text-center relative z-20 pointer-events-auto">
                    ...
                    >
                        ...
                        <span className="text-xs font-mono text-nofx-gold tracking-widest">
                            {language === 'zh'
                                ? `${BRAND_INFO.name.toUpperCase()} AI 交易系统`
                                : `${BRAND_INFO.name.toUpperCase()} AGENTIC TRADING OS`}
                        </span>
                    </motion.div>
                    ...
                    <div className="relative z-20 mix-blend-hard-light md:mix-blend-normal">
                        <h1 className="text-5xl sm:text-6xl md:text-8xl lg:text-9xl font-black tracking-tighter leading-[0.9] md:leading-[0.8] mb-6 select-none bg-clip-text text-transparent bg-gradient-to-b from-[var(--text-primary)] via-[var(--text-primary)] to-[var(--text-tertiary)] drop-shadow-2xl">
                            {language === 'zh' ? '智能代理' : 'AGENTIC'}<br />
                        ...
                        <p className="max-w-xl text-nofx-text-main md:text-nofx-text-muted text-lg mb-6 font-light leading-relaxed drop-shadow-md">
                            {language === 'zh'
                                ? '面向实盘的 AI 交易操作系统。基于先进大模型，部署可自主运行的高频交易代理。'
                                : "The world's first agentic trading OS. Deploy autonomous high-frequency trading agents powered by advanced LLMs."}
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col gap-4 mb-14">
                        <div className="text-nofx-gold/80 font-mono text-xs tracking-[0.3em] uppercase flex items-center gap-2 ml-1">
                            ...
                            {language === 'zh' ? '实时行情已接入' : 'Live Data Feeds Active'}
                        </div>
                        <div className="flex flex-wrap gap-4 font-mono">
                            {['CRYPTO', 'US STOCKS', 'FOREX', 'METALS'].map((market) => (
                                <div key={market} className="relative group cursor-default">
                                    ...
                                    <div className="relative flex items-center gap-3 px-6 py-3 rounded-lg bg-[var(--panel-bg)]/90 border border-[var(--glass-border)] hover:border-nofx-gold/50 transition-all duration-300 backdrop-blur-sm">
                                        <div className="w-1.5 h-1.5 rounded-full bg-nofx-success shadow-[0_0_8px_rgba(74,222,128,0.6)] animate-pulse"></div>
                                        <span className="text-lg md:text-xl font-bold text-nofx-text-main tracking-wider group-hover:text-nofx-gold transition-colors">{market}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
                        ...
                        >
                            <span className="relative z-10 flex items-center gap-2">
                                {language === 'zh' ? '启动协议' : 'INITIALIZE PROTOCOL'} <ArrowRight className="w-4 h-4" />
                            </span>
