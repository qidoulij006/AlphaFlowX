...
export default function LiveFeed() {
    ...
    return (
        <section className="w-full bg-nofx-bg-deeper border-y border-[var(--glass-border)] py-1 overflow-hidden relative">
            ...
            <div className="max-w-[1920px] mx-auto px-4 flex flex-col md:flex-row gap-0 md:gap-8 items-stretch h-[240px] md:h-12 text-xs font-mono">
                ...
                <div className="hidden md:flex items-center gap-6 text-[var(--text-tertiary)] border-r border-[var(--glass-border)] pr-6 shrink-0">
                    <div className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></div>
                        <span className="font-bold text-nofx-text-muted">{language === 'zh' ? '连接状态：稳定' : 'WS_CONN: STABLE'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                ...
                <div className="flex-1 overflow-hidden relative font-mono text-[10px] md:text-sm h-full flex items-center">
                            ...
                            >
                                ...
                                <span className={`font-bold w-10 ${log.type === 'LIQ' ? 'text-red-500 bg-red-500/10 px-1 rounded' :
                                    log.type === 'ARB' ? 'text-nofx-gold bg-nofx-gold/10 px-1 rounded' :
                                        log.type === 'EXE' ? 'text-green-500' : 'text-nofx-text-muted'
                                    }`}>{log.type}</span>
                    ...
                    <div className="md:hidden flex flex-col gap-2 w-full p-4 h-full overflow-hidden">
                        {logs.map((log) => (
                            <div key={log.id} className="flex gap-2 w-full truncate border-b border-[var(--glass-border)] pb-1 last:border-0">
                                <span className="text-[var(--text-disabled)] w-16 shrink-0">{log.time.split('.')[0]}</span>
