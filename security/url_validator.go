...
// ValidateProxyURL checks whether a user-supplied proxy URL is structurally valid
// and does not point to localhost, private networks, or metadata endpoints.
func ValidateProxyURL(rawURL string) error {
	...
	parsedURL, err := url.Parse(rawURL)
	if err != nil {
		return &SSRFError{URL: rawURL, Reason: "invalid proxy URL format"}
	}
	...
	default:
		return &SSRFError{URL: rawURL, Reason: fmt.Sprintf("unsupported proxy scheme: %s", scheme)}
	}

	if parsedURL.Hostname() == "" {
		return &SSRFError{URL: rawURL, Reason: "empty proxy hostname"}
	}

	validationURL := "http://" + parsedURL.Host
	return ValidateURL(validationURL)
}
