...
type Store struct {
	...
	equity         *EquityStore
	order          *OrderStore
	traderReview   *TraderReviewStore
	grid           *GridStore
	auditLog       *AuditLogStore
	shareLink      *ShareLinkStore
...
func (s *Store) initTables() error {
	...
	if err := s.ProxyServer().initTables(); err != nil {
		return fmt.Errorf("failed to initialize proxy server tables: %w", err)
	}
	...
	if err := s.TraderReview().initTables(); err != nil {
		return fmt.Errorf("failed to initialize trader review tables: %w", err)
	}
	...
	if err := s.ShareLink().initTables(); err != nil {
		return fmt.Errorf("failed to initialize share link tables: %w", err)
	}
...
func (s *Store) initDefaultData() error {
	...
	if err := s.Exchange().MigrateLegacyProxyURLs(s.ProxyServer()); err != nil {
		logger.Warnf("failed to migrate exchange proxy URLs: %v", err)
	}
...
// ProxyServer gets proxy server storage
func (s *Store) ProxyServer() *ProxyServerStore {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.proxyServer == nil {
		s.proxyServer = NewProxyServerStore(s.gdb)
	}
...
// ShareLink gets share link storage
func (s *Store) ShareLink() *ShareLinkStore {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.shareLink == nil {
		s.shareLink = NewShareLinkStore(s.gdb)
	}
	return s.shareLink
}
...
// TraderReview gets trader review storage
func (s *Store) TraderReview() *TraderReviewStore {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.traderReview == nil {
		s.traderReview = NewTraderReviewStore(s.gdb)
	}
	return s.traderReview
}
...
func (s *Store) AuditLog() *AuditLogStore {
	...
	defer s.mu.Unlock()
	if s.auditLog == nil {
		s.auditLog = NewAuditLogStore(s.gdb)
	}
	return s.auditLog
}
