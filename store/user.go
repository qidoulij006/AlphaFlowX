...
type User struct {
	...
	SessionRevokedAt *time.Time `gorm:"column:session_revoked_at" json:"session_revoked_at,omitempty"`
...
func (s *UserStore) initTables() error {
	...
	if s.db.Dialector.Name() == "postgres" {
		...
		if tableExists > 0 {
			...
			s.db.Exec(`ALTER TABLE users ADD COLUMN IF NOT EXISTS is_active BOOLEAN NOT NULL DEFAULT TRUE`)
			s.db.Exec(`ALTER TABLE users ADD COLUMN IF NOT EXISTS session_revoked_at TIMESTAMP NULL`)
			s.db.Exec(`ALTER TABLE users ADD COLUMN IF NOT EXISTS archived_at TIMESTAMP NULL`)
...
// CountCreatedAfter returns number of users created on or after the given time.
func (s *UserStore) CountCreatedAfter(since time.Time) (int, error) {
	var count int64
	err := s.db.Model(&User{}).Where("created_at >= ?", since).Count(&count).Error
	return int(count), err
}
...
// UpdateActive updates whether a user account is active.
func (s *UserStore) UpdateActive(userID string, isActive bool) error {
	return s.db.Model(&User{}).Where("id = ?", userID).Updates(map[string]interface{}{
		"is_active":  isActive,
		"updated_at": time.Now().UTC(),
	}).Error
...
// RevokeSessions invalidates all existing JWTs for a user issued before now.
...
// SetArchived toggles user archive state and keeps archived accounts disabled.
func (s *UserStore) SetArchived(userID string, archived bool) error {
	now := time.Now().UTC()
	...
	if archived {
		...
		updates["session_revoked_at"] = now
	} else {
...
// DeleteByID permanently removes a user row.
func (s *UserStore) DeleteByID(userID string) error {
