...
type Trader struct {
	...
	ReviewEnabled       bool      `gorm:"column:review_enabled;default:false" json:"review_enabled"`
	ReviewWindowMinutes int       `gorm:"column:review_window_minutes;default:240" json:"review_window_minutes"`
	ReviewTargetReturnPct float64 `gorm:"column:review_target_return_pct;default:2" json:"review_target_return_pct"`
	ReviewMaxLossProfitRatio float64 `gorm:"column:review_max_loss_profit_ratio;default:0.3333333333" json:"review_max_loss_profit_ratio"`
...
// ListAll returns all traders across all users ordered by creation time descending.
...
// CountAll returns the total number of traders.
func (s *TraderStore) CountAll() (int, error) {
	var count int64
	err := s.db.Model(&Trader{}).Count(&count).Error
	return int(count), err
}

// CountRunning returns the number of currently running traders.
func (s *TraderStore) CountRunning() (int, error) {
	var count int64
	err := s.db.Model(&Trader{}).Where("is_running = ?", true).Count(&count).Error
	return int(count), err
}
...
func (s *TraderStore) Update(trader *Trader) error {
	...
	updates := map[string]interface{}{
		...
		"review_enabled":            trader.ReviewEnabled,
	}
	...
	if trader.ReviewWindowMinutes > 0 {
		updates["review_window_minutes"] = trader.ReviewWindowMinutes
	}
	if trader.ReviewTargetReturnPct > 0 {
		updates["review_target_return_pct"] = trader.ReviewTargetReturnPct
	}
	if trader.ReviewMaxLossProfitRatio > 0 {
		updates["review_max_loss_profit_ratio"] = trader.ReviewMaxLossProfitRatio
	}
...
// UpdateStrategyID updates the linked strategy for a trader.
