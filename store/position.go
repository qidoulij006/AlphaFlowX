...
// GetLatestOpenPositionBySymbolAndSide gets the latest open position for a symbol/side,
// optionally filtering by source.
func (s *PositionStore) GetLatestOpenPositionBySymbolAndSide(traderID, symbol, side string, source string) (*TraderPosition, error) {
	var pos TraderPosition

	query := s.db.Where("trader_id = ? AND symbol = ? AND side = ? AND status = ?", traderID, symbol, side, "OPEN")
	if source != "" {
		query = query.Where("source = ?", source)
	}

	err := query.Order("entry_time DESC").First(&pos).Error
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, nil
		}
...
// UpsertSnapshotOpenPosition refreshes or creates a recovery snapshot position without
// disturbing normal fill-derived open positions.
func (s *PositionStore) UpsertSnapshotOpenPosition(pos *TraderPosition) error {
	if pos == nil {
		return nil
	}

	existing, err := s.GetLatestOpenPositionBySymbolAndSide(pos.TraderID, pos.Symbol, pos.Side, "snapshot")
	if err != nil {
		return fmt.Errorf("failed to query existing snapshot position: %w", err)
	}
	...
	updates := map[string]interface{}{
		...
		"exchange_position_id": pos.ExchangePositionID,
		"quantity":             pos.Quantity,
	...
	return s.db.Model(&TraderPosition{}).Where("id = ?", existing.ID).Updates(updates).Error
}

// CloseOpenSnapshotPosition marks an OPEN snapshot position as closed because the live
// exchange position is no longer present.
func (s *PositionStore) CloseOpenSnapshotPosition(id int64, closeReason string) error {
	if closeReason == "" {
		closeReason = "snapshot_sync"
	}
	nowMs := time.Now().UTC().UnixMilli()
	return s.db.Model(&TraderPosition{}).Where("id = ? AND status = ? AND source = ?", id, "OPEN", "snapshot").Updates(map[string]interface{}{
		"status":       "CLOSED",
		"exit_time":    nowMs,
		"close_reason": closeReason,
		"updated_at":   nowMs,
