...
type Exchange struct {
	...
	ProxyURL                crypto.EncryptedString `gorm:"column:proxy_url;default:''" json:"proxyUrl"`
	ProxyServerID           string                 `gorm:"column:proxy_server_id;default:'';index" json:"proxy_server_id"`
...
func (s *ExchangeStore) ensureProxyServerIDColumn() {
	if s.db.Migrator().HasColumn(&Exchange{}, "proxy_server_id") {
		return
	}
	if err := s.db.Migrator().AddColumn(&Exchange{}, "ProxyServerID"); err != nil {
		logger.Warnf("failed to add exchanges.proxy_server_id column: %v", err)
	}
}
...
// ListAll returns all exchanges across all users.
func (s *ExchangeStore) ListAll() ([]*Exchange, error) {
	var exchanges []*Exchange
	err := s.db.Order("updated_at DESC, created_at DESC").Find(&exchanges).Error
	if err != nil {
		return nil, err
	}
	return exchanges, nil
...
// CountAll returns the total number of exchange accounts.
func (s *ExchangeStore) CountAll() (int, error) {
	var count int64
	err := s.db.Model(&Exchange{}).Count(&count).Error
	return int(count), err
}

// CountEnabled returns the number of enabled exchange accounts.
func (s *ExchangeStore) CountEnabled() (int, error) {
	var count int64
	err := s.db.Model(&Exchange{}).Where("enabled = ?", true).Count(&count).Error
	return int(count), err
}
...
// GetByIDAny gets a specific exchange by UUID without requiring userID.
// Intended for internal runtime lookups where exchange IDs are already trusted.
...
func (s *ExchangeStore) GetProxyURLForExchange(id string) (string, error) {
	exchange, err := s.GetByIDAny(id)
	if err != nil {
		return "", err
	}
	if strings.TrimSpace(exchange.ProxyServerID) != "" {
		var proxy ProxyServer
		err := s.db.Where("id = ? AND user_id = ? AND enabled = ?", exchange.ProxyServerID, exchange.UserID, true).First(&proxy).Error
		if err != nil {
			return "", err
		}
		return string(proxy.ProxyURL), nil
	}
	return string(exchange.ProxyURL), nil
}

func (s *ExchangeStore) ProxyServerInUse(userID, proxyServerID, exceptExchangeID string) (bool, error) {
	if strings.TrimSpace(proxyServerID) == "" {
		return false, nil
	}
	var count int64
	query := s.db.Model(&Exchange{}).Where("user_id = ? AND proxy_server_id = ?", userID, proxyServerID)
	if exceptExchangeID != "" {
		query = query.Where("id <> ?", exceptExchangeID)
	}
...
func (s *ExchangeStore) Create(userID, exchangeType, accountName string, enabled bool,
	apiKey, secretKey, passphrase, proxyURL, proxyServerID string, testnet bool,
	...
	if inUse, err := s.ProxyServerInUse(userID, proxyServerID, ""); err != nil {
		return "", err
	} else if inUse {
		return "", fmt.Errorf("proxy server already bound to another exchange")
	}
...
func (s *ExchangeStore) Update(userID, id string, enabled bool, apiKey, secretKey, passphrase string, testnet bool,
	...
	if inUse, err := s.ProxyServerInUse(userID, proxyServerID, id); err != nil {
		return err
	} else if inUse {
		return fmt.Errorf("proxy server already bound to another exchange")
	}
...
func (s *ExchangeStore) CountByProxyServerID(userID, proxyServerID string) (int64, error) {
	var count int64
	err := s.db.Model(&Exchange{}).Where("user_id = ? AND proxy_server_id = ?", userID, proxyServerID).Count(&count).Error
	return count, err
}

func (s *ExchangeStore) MigrateLegacyProxyURLs(proxyStore *ProxyServerStore) error {
	if proxyStore == nil {
		return nil
	}
	var exchanges []Exchange
	if err := s.db.Where("proxy_server_id = '' AND proxy_url <> ''").Find(&exchanges).Error; err != nil {
		return err
	}
	for _, exchange := range exchanges {
		proxyURL := strings.TrimSpace(string(exchange.ProxyURL))
		if proxyURL == "" {
			continue
		}
		name := fmt.Sprintf("%s %s proxy", exchange.AccountName, exchange.ExchangeType)
		proxyID, err := proxyStore.Create(exchange.UserID, name, proxyURL, "")
		if err != nil {
			return err
		}
		if err := s.db.Model(&Exchange{}).Where("id = ?", exchange.ID).Update("proxy_server_id", proxyID).Error; err != nil {
			return err
		}
		logger.Infof("Migrated legacy proxy_url for exchange %s into proxy server %s", exchange.ID, proxyID)
	}
	return nil
}
