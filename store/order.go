...
// SyncLiveOpenOrdersSnapshot replaces the local open-order mirror for a trader+symbol
// with the latest successful exchange snapshot. Only NEW/PARTIALLY_FILLED rows are touched.
func (s *OrderStore) SyncLiveOpenOrdersSnapshot(traderID, exchangeID, exchangeType, symbol string, liveOrders []*TraderOrder) error {
	nowMs := time.Now().UTC().UnixMilli()

	return s.db.Transaction(func(tx *gorm.DB) error {
		var existing []*TraderOrder
		if err := tx.Where("trader_id = ? AND symbol = ? AND status IN ?", traderID, symbol, []string{"NEW", "PARTIALLY_FILLED"}).
			Find(&existing).Error; err != nil {
			return fmt.Errorf("failed to load existing live orders: %w", err)
		}

		existingByExchangeOrderID := make(map[string]*TraderOrder, len(existing))
		for _, order := range existing {
			existingByExchangeOrderID[order.ExchangeOrderID] = order
		}

		seen := make(map[string]bool, len(liveOrders))
		for _, liveOrder := range liveOrders {
			if liveOrder == nil || liveOrder.ExchangeOrderID == "" {
				continue
			}
			seen[liveOrder.ExchangeOrderID] = true

			if current := existingByExchangeOrderID[liveOrder.ExchangeOrderID]; current != nil {
				updates := map[string]interface{}{
					"exchange_type":    exchangeType,
					"client_order_id":  liveOrder.ClientOrderID,
					"side":             liveOrder.Side,
					"position_side":    liveOrder.PositionSide,
					"type":             liveOrder.Type,
					"time_in_force":    liveOrder.TimeInForce,
					"quantity":         liveOrder.Quantity,
					"price":            liveOrder.Price,
					"stop_price":       liveOrder.StopPrice,
					"status":           liveOrder.Status,
					"filled_quantity":  liveOrder.FilledQuantity,
					"avg_fill_price":   liveOrder.AvgFillPrice,
					"commission":       liveOrder.Commission,
					"commission_asset": liveOrder.CommissionAsset,
					"leverage":         liveOrder.Leverage,
					"reduce_only":      liveOrder.ReduceOnly,
					"close_position":   liveOrder.ClosePosition,
					"working_type":     liveOrder.WorkingType,
					"price_protect":    liveOrder.PriceProtect,
					"order_action":     liveOrder.OrderAction,
					"updated_at":       nowMs,
				}
				if err := tx.Model(&TraderOrder{}).Where("id = ?", current.ID).Updates(updates).Error; err != nil {
					return fmt.Errorf("failed to update live order %s: %w", liveOrder.ExchangeOrderID, err)
				}
				continue
			}
			...
			newOrder := *liveOrder
			newOrder.ID = 0
			newOrder.TraderID = traderID
			newOrder.ExchangeID = exchangeID
			newOrder.ExchangeType = exchangeType
			newOrder.Symbol = symbol
			if newOrder.Status == "" {
				newOrder.Status = "NEW"
			}
			if newOrder.CreatedAt == 0 {
				newOrder.CreatedAt = nowMs
			}
			newOrder.UpdatedAt = nowMs
			if err := tx.Create(&newOrder).Error; err != nil {
				return fmt.Errorf("failed to create live order %s: %w", liveOrder.ExchangeOrderID, err)
			}
		}

		for _, staleOrder := range existing {
			if seen[staleOrder.ExchangeOrderID] {
				continue
			}
			if err := tx.Model(&TraderOrder{}).Where("id = ?", staleOrder.ID).Updates(map[string]interface{}{
				"status":     "CANCELED",
				"updated_at": nowMs,
			}).Error; err != nil {
				return fmt.Errorf("failed to mark stale live order %s as canceled: %w", staleOrder.ExchangeOrderID, err)
			}
		}
...
// GetTraderFillsByTimeRange returns fills created within the given Unix ms window.
func (s *OrderStore) GetTraderFillsByTimeRange(traderID string, startMs, endMs int64) ([]*TraderFill, error) {
	var fills []*TraderFill
	err := s.db.Where("trader_id = ? AND created_at >= ? AND created_at <= ?", traderID, startMs, endMs).
		Order("created_at ASC").
		Find(&fills).Error
	if err != nil {
		return nil, fmt.Errorf("failed to query fills by time range: %w", err)
	}
	return fills, nil
}

// GetRecentTraderFills gets the most recent fills for a trader, optionally filtered by symbol.
func (s *OrderStore) GetRecentTraderFills(traderID string, symbol string, limit int) ([]*TraderFill, error) {
	var fills []*TraderFill
	query := s.db.Where("trader_id = ?", traderID)
	...
	err := query.Order("created_at DESC").
		...
		Find(&fills).Error
	if err != nil {
		return nil, fmt.Errorf("failed to query recent fills: %w", err)
	}
	return fills, nil
}
