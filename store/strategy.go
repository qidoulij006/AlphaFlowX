...
type CoinSourceConfig struct {
	...
	// Note: external quant source settings are preserved for backward compatibility
}
...
type IndicatorConfig struct {
	...
	// ========== External Quant Data Configuration ==========
	// External quant data key (new neutral field)
	ExternalDataKey string `json:"external_data_key,omitempty"`
	// Legacy field kept for backward compatibility with old strategy JSON
...
func (c *IndicatorConfig) normalizeExternalDataKey() {
	if c.ExternalDataKey == "" {
		c.ExternalDataKey = c.NofxOSAPIKey
	}
	if c.NofxOSAPIKey == "" {
		c.NofxOSAPIKey = c.ExternalDataKey
	}
}

func (c *IndicatorConfig) GetExternalDataKey() string {
	c.normalizeExternalDataKey()
	return c.ExternalDataKey
}

func (c *IndicatorConfig) SetExternalDataKey(value string) {
	c.ExternalDataKey = value
	c.NofxOSAPIKey = value
}

func (c *IndicatorConfig) UnmarshalJSON(data []byte) error {
...
func (c IndicatorConfig) MarshalJSON() ([]byte, error) {
	c.normalizeExternalDataKey()
	type Alias IndicatorConfig
...
func (s *StrategyStore) initDefaultData() error {
	...
	return s.migrateExternalDataKeyCompatibility()
}

func normalizeStrategyConfigJSON(raw string) (string, bool, error) {
	if raw == "" {
	...
	indicators, ok := root["indicators"].(map[string]any)
	if !ok || indicators == nil {
		return raw, false, nil
	}

	externalKey, hasExternal := indicators["external_data_key"]
	legacyKey, hasLegacy := indicators["nofxos_api_key"]

	changed := false
	switch {
	case !hasExternal && hasLegacy:
		indicators["external_data_key"] = legacyKey
		changed = true
	case hasExternal && !hasLegacy:
		indicators["nofxos_api_key"] = externalKey
		changed = true
	}
...
func (s *StrategyStore) migrateExternalDataKeyCompatibility() error {
	...
	for _, strategy := range strategies {
		normalized, changed, err := normalizeStrategyConfigJSON(strategy.Config)
		if err != nil {
			logger.Warnf("⚠️ Failed to normalize strategy config for %s: %v", strategy.ID, err)
			continue
		}
		if !changed {
			continue
	...
	if migrated > 0 {
		logger.Infof("✅ Backfilled external_data_key compatibility for %d strategies", migrated)
	}
