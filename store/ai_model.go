...
// ListAll returns all AI models across all users.
func (s *AIModelStore) ListAll() ([]*AIModel, error) {
	var models []*AIModel
	err := s.db.Order("updated_at DESC, created_at DESC").Find(&models).Error
	if err != nil {
		return nil, err
	}
	return models, nil
...
// CountAll returns the total number of AI model configs.
func (s *AIModelStore) CountAll() (int, error) {
	var count int64
	err := s.db.Model(&AIModel{}).Count(&count).Error
	return int(count), err
}

// CountEnabled returns the number of enabled AI model configs.
func (s *AIModelStore) CountEnabled() (int, error) {
	var count int64
	err := s.db.Model(&AIModel{}).Where("enabled = ?", true).Count(&count).Error
	return int(count), err
}
...
func (s *AIModelStore) Update(userID, id, explicitProvider, displayName string, enabled bool, apiKey, customAPIURL, customModelName string) error {
	...
	provider := inferAIProvider(userID, id, explicitProvider)
	if id == provider {
	...
	if displayName != "" {
		name = displayName
	} else if err := s.db.Where("provider = ?", provider).First(&refModel).Error; err == nil {
		name = refModel.Name
	} else {
...
func inferAIProvider(userID, id, explicitProvider string) string {
	if explicitProvider != "" {
		return explicitProvider
	}

	if _, ok := supportedAIProviders[id]; ok {
		return id
	}
	...
	if userID != "" {
		...
		if strings.HasPrefix(id, prefix) {
			...
			for provider := range supportedAIProviders {
				if remainder == provider || strings.HasPrefix(remainder, provider+"_") {
					return provider
				}
			}
			if remainder != "" {
				return strings.SplitN(remainder, "_", 2)[0]
			}
		}
	}
	...
	if len(parts) >= 2 {
		for i := len(parts) - 1; i >= 0; i-- {
			if _, ok := supportedAIProviders[parts[i]]; ok {
				return parts[i]
			}
		}
	}
