...
// GetClosedPositionsByTimeRange returns closed positions exited within the given Unix ms window.
func (s *PositionStore) GetClosedPositionsByTimeRange(traderID string, startMs, endMs int64) ([]*TraderPosition, error) {
	var positions []*TraderPosition
	err := s.db.Where("trader_id = ? AND status = ? AND exit_time >= ? AND exit_time <= ?", traderID, "CLOSED", startMs, endMs).
		Order("exit_time ASC").
		Find(&positions).Error
	if err != nil {
		return nil, fmt.Errorf("failed to query closed positions by time range: %w", err)
	}
