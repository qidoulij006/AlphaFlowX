...
func (s *Server) handleGetGridRiskHistory(c *gin.Context) {
	traderID := c.Param("id")
	...
	if limitParam := strings.TrimSpace(c.Query("limit")); limitParam != "" {
		if parsed, parseErr := strconv.Atoi(limitParam); parseErr == nil && parsed > 0 && parsed <= 200 {
			limit = parsed
		}
	}

	c.JSON(http.StatusOK, gin.H{
		"items": autoTrader.GetGridRiskHistory(limit),
	})
}
