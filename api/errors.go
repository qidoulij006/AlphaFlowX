...
func SafeInternalError(c *gin.Context, operation string, err error) {
	if IsRateLimitError(err) {
		SafeRateLimited(c, operation, err)
		return
	}
...
// SafeRateLimited returns a safe rate-limit error when an upstream exchange throttles requests.
func SafeRateLimited(c *gin.Context, operation string, err error) {
	logger.Errorf("[Rate Limited] %s: %v", operation, err)
...
// IsRateLimitError checks whether an upstream dependency rejected the request due to throttling.
