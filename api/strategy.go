...
func validateStrategyConfig(config *store.StrategyConfig) []string {
	...
	// Warn when legacy external quant features remain enabled without a key
	if (config.Indicators.EnableQuantData || config.Indicators.EnableOIRanking ||
		...
		config.Indicators.ExternalDataKey == "" && config.Indicators.NofxOSAPIKey == "" {
		warnings = append(warnings, "External quant data is disabled or not configured in this deployment.")
	}
