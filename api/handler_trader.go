...
func (s *Server) handleCreateTrader(c *gin.Context) {
	...
	if exchangeCfg == nil {
		...
		case "binance":
			binanceTrader := binance.NewFuturesTrader(string(exchangeCfg.APIKey), string(exchangeCfg.SecretKey), userID, exchangeCfg.ID)
			binanceTrader.BindCooldownStore(s.store, exchangeCfg.ID)
	...
	// Immediately load and start the new trader so creation matches runtime state.
	...
	if err != nil {
		...
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Trader created but failed to load into runtime: " + err.Error(), "trader_id": traderID})
		return
	}
	logger.Infof("🔧 DEBUG: LoadUserTraders completed")

	loadedTrader, loadErr := s.traderManager.GetTrader(traderID)
	if loadErr != nil {
		if err := s.store.Trader().UpdateStatus(userID, traderID, false); err != nil {
			logger.Infof("⚠️ Failed to reset trader running status after load failure: %v", err)
		}
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Trader created but failed to initialize runtime trader", "trader_id": traderID})
		return
	}

	go func() {
		logger.Infof("▶️  Auto-starting newly created trader %s (%s)", traderID, loadedTrader.GetName())
		if runErr := loadedTrader.Run(); runErr != nil {
			logger.Infof("❌ Auto-start trader %s runtime error: %v", loadedTrader.GetName(), runErr)
		}
	}()

	if err := s.store.Trader().UpdateStatus(userID, traderID, true); err != nil {
		logger.Infof("⚠️ Failed to update trader running status after auto-start: %v", err)
	}

	logger.Infof("✓ Trader created and auto-started successfully: %s (model: %s, exchange: %s)", req.Name, req.AIModelID, req.ExchangeID)
