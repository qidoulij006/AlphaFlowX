...
func (s *Server) handleTraderList(c *gin.Context) {
	...
	for _, trader := range traders {
		...
		errorHint := s.getTraderErrorHint(trader.ID)
		result = append(result, map[string]interface{}{
			...
			"error_hint_code":     valueOrNil(errorHint, func(h *traderErrorHint) string { return h.Code }),
			"error_hint":          valueOrNil(errorHint, func(h *traderErrorHint) string { return h.Message }),
			"error_message":       valueOrNil(errorHint, func(h *traderErrorHint) string { return h.Raw }),
		})
	}
...
func valueOrNil[T any](value *T, getter func(*T) string) interface{} {
	if value == nil {
		return nil
	}
	result := getter(value)
...
// handleRecentFills Recent fill list for dashboard display
func (s *Server) handleRecentFills(c *gin.Context) {
	_, traderID, err := s.getTraderFromQuery(c)
	if err != nil {
		SafeBadRequest(c, "Invalid trader ID")
		return
	...
	fills, err := store.Order().GetRecentTraderFills(trader.GetID(), symbol, limit)
	if err != nil {
		SafeInternalError(c, "Get recent fills", err)
		return
	}
...
// handleGridPairs Grid inventory lot pairing view for dashboard
