...
func Get(symbol string) (*Data, error) {
	return GetWithExchangeAccount(symbol, "binance", "")
}
...
func GetWithExchange(symbol, exchange string) (*Data, error) {
	return GetWithExchangeAccount(symbol, exchange, "")
}

// GetWithExchangeAccount retrieves market data and uses the exchange account's HTTP settings when provided.
func GetWithExchangeAccount(symbol, exchange, exchangeID string) (*Data, error) {
	...
	// Get OI data
	oiData, err := getOpenInterestData(symbol, exchangeID)
	if err != nil {
	...
	// Get Funding Rate
	fundingRate, _ := getFundingRate(symbol, exchangeID)
...
func GetWithTimeframes(symbol string, timeframes []string, primaryTimeframe string, count int) (*Data, error) {
	...
	var longerKlines []Kline
	longestTimeframeMinutes := 0
	...
	for _, tf := range timeframes {
		...
		primaryMinutes := parseTimeframeToMinutes(primaryTimeframe)
		if tf != primaryTimeframe && tfMinutes > primaryMinutes && tfMinutes >= longestTimeframeMinutes {
			longerKlines = klines
			longestTimeframeMinutes = tfMinutes
		}
	...
	if len(longerKlines) > 0 {
		data.LongerTermContext = calculateLongerTermData(longerKlines)
	}
...
func getOpenInterestData(symbol, exchangeID string) (*OIData, error) {
	url := fmt.Sprintf("https://fapi.binance.com/fapi/v1/openInterest?symbol=%s", symbol)

	apiClient := NewAPIClient(exchangeID)
...
func getFundingRate(symbol, exchangeID string) (float64, error) {
